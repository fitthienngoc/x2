...
export default class Person extends Component {
    ...
    showMore_csAn(person) {
        var { height } = this.state
        if (this.state.seeMore) {
            return <>
                {this.state.seeMoreCsAn ? <div>
                    {person.csAn.map((item, index) => {
                        console.log('item.img', item.img)
                        return <a key={index} className="profile-card-social__item twitter tooltip" target="_blank">
                            <img style={{ padding: 15 }} src={item.img} />
                            <span style={{ marginTop: -95, minWidth: height * 0.4 }} className="tooltiptext">{item.direction}</span>
                        </a>
                ...
                <button onClick={() => { this.setState({ seeMoreCsAn: !this.state.seeMoreCsAn }) }} className="profile-card__button button--blue" style={{
                    fontSize: 17
                }}>{this.state.seeMoreCsAn ? 'Ẩn' : 'Chỉ Số Ẩn'}</button>
            </>
        }
    ...
    show_(item) {
        ...
        for (let [key, value] of Object.entries(item)) {
            return <div className="profile-card-inf__item">
                {key == 'name' ? <div className="profile-card-inf__title">{value}</div> :
                    <div className="profile-card-inf__txt">{value}</div>}
            </div>
        }
    }
    showMore() {
        return <button
            ...
            }}>{this.state.seeMore ? 'Ẩn' : 'Xem Thêm'}</button>
    }
    compare(a, b) {
        // Dùng toUpperCase() để không phân biệt ký tự hoa thường
        const genreA = a.chiSo
        const genreB = b.chiSo
    ...
    showChiSo(person) {
        var r = ''
        var z = 100
        if (person && person.csChung)

            r = person.csChung.sort(this.compare).map((item, index) => {
                if (!this.state.seeMore) { z = 3 }
                if (index <= z)
                    return <div key={index} className="profile-card-inf__item">
                        <div className="profile-card-inf__title">{item.chiSo}</div>
                        <div className="profile-card-inf__txt">{item.name}</div>
                    </div>
            }
    ...
    showPlayer(person) {
        ...
        return <div className="modal">
            <div style={{ position: 'fixed', right: 30, zIndex: 100 }}>
                <a onClick={() => { this.onTogleShowPlayer(person) }} className="profile-card-social__item link " target="_blank">
                    Close
                </a>
            </div>

            <div>
                <div className="wrapper" style={{ height: height * 0.9 }}>

                    <div className="profile-card js-profile-card">

                        <div className="profile-card__img">
                            <img src={`https://graph.facebook.com/${person.idFb}/picture?type=large`} alt="profile card" />
                        </div>

                        <div className="profile-card__cnt js-profile-cnt">
                            <div className="profile-card__name">{person.name} - {person.number}</div>
                            <div className="profile-card__txt"><strong>{person.birhday}</strong> </div>
                            <div className="profile-card__txt"><strong>{person.tall} cm - {person.canNang} kg</strong> </div>
                            <div className="profile-card-loc">
                                <span className="profile-card-loc__icon">

                                    <img style={{ width: height * 0.2, height: height * 0.2 }} src='/static/t-shirt.png' />
                            ...
                            <div className="profile-card-inf">
                                ...
                                {this.showChiSo(person)}


                            </div>
                            {this.showMore()}
                            {this.showMore_csAn(person)}
                            <div className="profile-card-social">
                                <a href={`https://www.facebook.com/${person.idFb}`} className="profile-card-social__item facebook" target="_blank">
    ...
    onTogleShowPlayer(person) {
        this.setState({ isModal: !this.state.isModal })
    }
    render() {
        var { height, person } = this.props
        return (
            <>
                <div style={{ height: height * 0.2, width: height * 0.12 }}>
                    <img onClick={() => { this.onTogleShowPlayer(person) }} className='player' style={{ height: height * 0.12, width: height * 0.12 }} src={`https://graph.facebook.com/${person.idFb}/picture?type=large`} />
                    <p className="namePlayer" style={{ fontSize: height * 0.02 }}>{person.name} {person.number}</p>
                </div>

                {this.state.isModal ? this.showPlayer(person) : null}
            </>
        )
    }
}
